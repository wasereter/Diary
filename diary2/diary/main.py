...
def register(win):
    ...
    tk.Label(win, text="ФИО:", width=50, height=2, font=14).pack()
    entry_fio = tk.Entry(win, width=50, font=14)
    entry_fio.pack()

    tk.Label(win, text="Группа:", width=50, height=2, font=14).pack()
    entry_group = tk.Entry(win, width=50, font=14)
    entry_group.pack()

    tk.Label(win, text="Пароль:", width=50, height=2, font=14).pack()
    entry_password = tk.Entry(win, show="*", width=50, font=14)
    entry_password.pack()

    role_var = tk.StringVar(value="student")  # Установка значения по умолчанию на "Ученик"
    tk.Radiobutton(win, text="Ученик", variable=role_var, value="student", width=50, height=2, font=14).pack()
    tk.Radiobutton(win, text="Учитель", variable=role_var, value="teacher", width=50, height=2, font=14).pack()
    tk.Radiobutton(win, text="Администратор", variable=role_var, value="admin", width=50, height=2, font=14).pack()

    tk.Button(win, text="Зарегистрироваться", command=submit, width=50, height=2, font=14, bg="#00ff00").pack()
...
def display_student(win):
    ...
    tk.Button(win, text="Средний балл", command=show_avg, bg="#00ff56").pack(pady=5)
...
def display_teacher(win):
    ...
    tk.Button(win, text="Добавить предмет", command=add_new_subject, width=30, height=2, font=14, bg="#0080ff").pack(pady=8)
    ...
    tk.Button(win, text="Добавить", command=submit_grade, height=2, width=30, font=14, bg="#0080ff").pack(pady = 8)
    tk.Button(win, text="Средний балл", command=show_avg, height=2, width=30, font=14, bg="#0080ff").pack(pady = 8)

tk.Label(root, text="Введите ФИО и пароль для входа:", font=14, bg="#ffffff").pack(pady=10)
entry_login = tk.Entry(root, width=40, font=14, bg="#00ffff")
entry_login.pack(pady=5)
entry_password = tk.Entry(root, width=40, show='*', font=14, bg="#00ffff")
entry_password.pack(pady=5)

tk.Button(root, text="Войти", command=login, width=20, height=2, bg="#ff0000", font="14").pack(pady=10)
